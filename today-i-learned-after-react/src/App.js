...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  function handleSubmit(e) {
    // 1. prevent browser form reloading
    e.preventDefault();

    // 2. check if data is valid, if it is, create new fact
    if (text && isValidHttpUrl(source) && category && textLength <= 200) {
      // 3. create new fact object
      const newFact = {
        id: Math.round(Math.random() * 1231293129),
        text,
        source,
        category,
        votesInteresting: 0,
      ...
      // 4. add new fact to the user inteface - add the fact to state
